...
def save_model_to_adios2(model, filepath, overwrite=True, include_optimizer=True):
    """Saves a model to a ADIOS BP file.
  ...
  Raises:
      ImportError: if adios2 is not available.
  """

    if adios2 is None:
        raise ImportError('`save_model` requires adios2.')
    ...
    with adios2.open(filepath, "w") as fh:
        # with adios2.open(filepath, "w", comm) as fh:
        ...
        try:
            ...
            for k, v in model_metadata.items():
                if isinstance(v, (dict, list, tuple)):
                    attributes[k] = json.dumps(
                        # v, default=json_utils.get_json_type).encode('utf8')
                        v, default=serialization.get_json_type).encode('utf8')
                else:
                    attributes[k] = v

            model_weights_group_path = delimiter + 'model_weights'
            optimizer_weights_group_path = ""
            model_layers = model.layers
            save_weights_to_adios2(fh, model_weights_group_path, model_layers)
            if (include_optimizer and model.optimizer and
                    not isinstance(model.optimizer, optimizers.TFOptimizer)):
                save_optimizer_weights_to_adios2(fh, optimizer_weights_group_path,  model.optimizer)
            fh.end_step()
...
def save_weights_to_adios2(fh, path, layers):
    """Saves the weights of a list of layers to a ADIOS2.
  ...
  """
    ...
    # TODODG add attributes
    # save_attributes_to_adios2(
    ...
    for layer in sorted(layers, key=lambda x: x.name):
        ...
        g = path + delimiter + layer.name
        weights = _legacy_weights(layer)
        weight_values = K.batch_get_value(weights)
        weight_names = [w.name.encode('utf8') for w in weights]
        #save_attributes_to_adios2(fh, g, 'weight_names', weight_names)

        for name, val in zip(weight_names, weight_values):
            if not val.shape:
                fh.write(path + delimiter + name.decode("utf-8"), np.array([val]))
            else:
                if len(val.shape) == 1:
                    nx = val.shape[0]
                    shape = [nx]
                    start = [0]
                    count = [nx]
                    fh.write(path + delimiter + name.decode("utf-8"), val, shape, start, count)
...
def save_optimizer_weights_to_adios2(fh, group_path, optimizer):
    """Saves optimizer weights of a optimizer to adios2 .
  ...
  """
    ...
    if symbolic_weights:
        weights_group = group_path + delimiter + 'optimizer_weights'
        weight_names = [str(w.name).encode('utf8') for w in symbolic_weights]
        # save_attributes_to_adios2(weights_group, 'weight_names', weight_names)
        weight_values = K.batch_get_value(symbolic_weights)
        for name, val in zip(weight_names, weight_values):
            if not val.shape:
                fh.write(weights_group + delimiter + name.decode("utf-8"), np.array([val]))
            else:
                if len(val.shape) == 1:
                    nx = val.shape[0]
                    shape = [nx]
                    start = [0]
                    count = [nx]
                    fh.write(weights_group + delimiter + name.decode("utf-8"), val, shape, start, count )
                elif len(val.shape) == 2:
                    nx = val.shape[0]
                    ny = val.shape[1]
                    shape = [nx, ny]
...
def save_attributes_to_adios2(group, name, data):
